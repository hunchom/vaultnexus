 ...
 *
 * `hedge`/`assertion` default to v1 lexicons → existing callers unaffected. Plan 21 harness passes
 * perturbed lexicons to measure Jaccard stability of the drift signal.
 */
export function conviction(
  text: string,
  hedge: readonly string[] = HEDGE_WORDS_V1,
  assertion: readonly string[] = ASSERTION_WORDS_V1,
): number {
  const total = wordCount(text);
  if (total === 0) return 0;
  const lower = text.toLowerCase();
  let hedges = 0;
  for (const w of hedge) hedges += countTerm(lower, w);
  let assertions = 0;
  for (const w of assertion) assertions += countTerm(lower, w);
...
/** Lexicon override for Plan 21 harness. Both fields required when supplied. */
export interface DriftLexicon {
  hedge: readonly string[];
 ...
 *
 * `lexicon` defaults to v1 → existing callers unaffected. Plan 21 passes perturbed lexicons.
 */
export function driftFlag(
  ...
  const hedge = lexicon?.hedge ?? HEDGE_WORDS_V1;
  const assertion = lexicon?.assertion ?? ASSERTION_WORDS_V1;
  const samples = revisions.map((r) => ({
    date: r.date,
    conviction: conviction(r.content, hedge, assertion),
