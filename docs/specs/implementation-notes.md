...
- **Ship 6 adapters + 1 generic:** Gemini (default), Voyage-4 (shared-space family), Nomic (Atlas), Cohere-v4 (128K ctx), OpenAI-3, Jina-v5-small (best <1B), **+ generic OpenAI-compatible** (base-URL+key+`EMBEDDING_DIMS` → covers EmbeddingGemma, Qwen3-Embedding, Llama-Embed-Nemotron, BGE-M3, nomic-v2-moe, voyage-4-nano). Add `EMBEDDING_TASK_DOC`/`EMBEDDING_TASK_QUERY` overrides for the generic adapter (open models disagree on prefix wording).
